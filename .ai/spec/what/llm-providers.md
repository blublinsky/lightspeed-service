...
13. The valid `reasoning_config` keys vary by provider and model generation:
    ...
    - **Anthropic (Vertex)**: `thinking_enabled` (bool). When true, enables thinking; when false or absent, no thinking config is sent. `budget_tokens` (int, optional) for thinking token budget. The provider constructs `model_kwargs["thinking"]` dict with `type: "enabled"` and `budget_tokens` for `ChatAnthropicVertex`.
...
14. Provider APIs return 400 errors if reasoning parameters are sent to non-reasoning models. There is no reliable cross-provider API to auto-detect reasoning capability at startup.
